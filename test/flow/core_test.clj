...
(defn- dec-number
  [resources data]
  (update data :number dec))

(defn- inc-number
  [resources data]
  (update data :number inc))
...
(deftest calculate-zero-flow
  (testing "number is positive"
    (let [data {:number 5}
          resources {}
          new-data (execute {:graph calculate-zero-graph
                             :start get-the-number}
                            resources
                            data)]
      (is (= new-data {:number 0}))))
  (testing "number is negative"
    (let [data {:number -5}
          resources {}
          new-data (execute {:graph calculate-zero-graph
                             :start get-the-number}
